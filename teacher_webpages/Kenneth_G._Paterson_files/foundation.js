...
(function(window, document, Granite, $) {
    ...
    function getConfig(config) {
        ...
        defaultConfig.ADD_ATTR = defaultConfig.ADD_ATTR.concat(config.ADD_ATTR || []);
        defaultConfig.ADD_TAGS = defaultConfig.ADD_TAGS.concat(config.ADD_TAGS || []);
    ...
    Granite.UI.Foundation.XSS = Granite.UI.Foundation.XSS || (function() {
        return {
            ...
            sanitizeHtml: function(html, config) {
                ...
                // @todo remove diff, once majority security bug has been resolved.
...
(function(document, $, Granite, Coral) {
    ...
    var maskEl = (function() {
        ...
        el.classList.add("foundation-ui-mask");
    ...
    registry.register("foundation.adapters", {
        ...
        adapter: function() {
            return {
                ...
                wait: function(el) {
                    (el || document.body).append(maskEl);
                },
...
(function(window, document, $, Coral, Granite, URITemplate) {
    ...
    Prototype._render = function() {
        ...
        if (this.droppable) {
            this._input.classList.add("foundation-autocomplete-droppable");
        }
    ...
    Prototype._onUserInput = function(skipSuggestions) {
        var self = this;
    ...
    Prototype._onChangeEventDeferred = (function() {
...
(function(window, document, $, URITemplate) {
    ...
    function resolveToggleable(control, src, target) {
        if (!src) {
            var el = target ? $(target) : control.closest(".foundation-toggleable");
...
(function(document, $) {
    ...
    $(document).on("click", ".foundation-collection-selectall", function(e) {
        ...
        var collection = document.querySelector(targetSelector);
        ...
        collection.dataset.foundationSelectionsSelectallMode = true;

        selectionAPI.selectAll();

        // wait for deselect button to appear and move focus to that button
        window.requestAnimationFrame(isDeSelectAvaiable);
    });

    function isDeSelectAvaiable() {
        var deselect = $("coral-actionbar-secondary >" +
                            "coral-actionbar-item:not([coral-actionbar-offscreen]) > .granite-collection-deselect");
        if (deselect.length) {
            deselect.focus();
        } else {
            window.requestAnimationFrame(isDeSelectAvaiable);
...
(function(document, window, $, Granite) {
    ...
    function showLoadingMask($form) {
        var form = $form[0];
        var mask = form.dataset.foundationFormLoadingmask;
        ...
        if (mask === "true") {
            var panel;
            var panelSelector = form.dataset.foundationFormOutputReplace || form.dataset.foundationFormOutputPush;

            if (panelSelector) {
                panel = document.querySelector(panelSelector);
            }

            form.dataset.foundationFormMaskLoaded = "true";
            ui.wait(panel);
        }
    }
    ...
    function clearLoadingMask($form) {
        ...
        if (form.dataset.foundationFormMaskLoaded === "true") {
            ui.clearWait();
            delete form.dataset.foundationFormMaskLoaded;
        }
    }
...
(function(window, $, Granite) {
    ...
    $(window).adaptTo("foundation-registry").register("foundation.form.response.ui.success", {
        ...
        handler: function(form, config, data, textStatus, xhr) {
            ...
            $("#granite-collection-switcher-toggle-button").focus();
        }
    });
})(window, Granite.$, Granite);
...
(function(window, $, Coral) {
    ...
    registry.register("foundation.adapters", {
        ...
        adapter: function(el) {
            ...
            $el.on("granite-columnview:initial-load-completed",
                ".foundation-collection.foundation-layout-columnview", function(e) {
                    var columns = this.columns.getAll();
                    this._scrollColumnIntoView(columns[columns.length - 1], false, true);
                });
...
(function(window, document, $) {
    ...
    $document.on("dialog-ready", function(e) {
        var form = document.querySelector("coral-dialog.cq-dialog-floating[open] form.cq-dialog");
        if (form) {
            var swatchOverlay = form.querySelector("coral-colorinput[variant=swatch] > coral-overlay");
            if (swatchOverlay) {
                swatchOverlay.within = form;
            }
        }
    });
})(window, document, Granite.$);
...
(function(window, $) {
    ...
    function traverse(parent, callback) {
        $(parent).children().each(function() {
            if ($(this).is(":-foundation-submittable")) {
                callback(this);
            } else {
    ...
    $(document).on("coral-collection:add", MULTIFIELD_SELECTOR, function(event) {
    ...
    $(document).on("coral-collection:add coral-collection:remove", COMPOSITE_MULTIFIELD_SELECTOR, function() {
        changeNames(this, "");
    });

    // We need to process itemorder event in different style
    // This is done to ensure correct value in radiobutton item used inside in multifield.
    $(document).on("coral-multifield:itemorder", COMPOSITE_MULTIFIELD_SELECTOR, function(event) {
...
(function(window, document, Granite, jquery, Coral, URITemplate) {
    ...
    /**
     * @typedef {Event} foundation-collection-sort:change
     *
     * This event is triggered when sorting parameter/values of collection change.
     */
     ...
     *
     * @param {State} state the state
     * @param {HTMLElement[]} itemEls and array of item elements
     ...
     * @returns {Promise} a promise object, will be resolved when sorting completes.
     */
    function performSort(state, sortConfig) {
        var collection = state.collection;
        var collectionEl = state.collectionEl;
        ...
        adapter: function(el) {
            ...
            return {
                ...
                isAllLoaded: function() {
                    ...
                    var lazyPendingItems = getLazyOrPendingItems(el);
                ...
                count: function() {
                    if (el.dataset.foundationSelectionsSelectallMode === "true") {
                        ...
                        if (paginationAPI && paginationAPI.guessTotal !== -1) {
                            ...
                            var unselectedItems = getUnselectedItems(el);
                            return paginationAPI.guessTotal - unselectedItems.length;
                        }
                    }
                    return getSelectedItems(el).length;
                },
...
(function(window, jquery, URITemplate) {
    ...
    function getCurrentColumn(collectionEl, includePreview) {
        var currentColumn = getColumnById(collectionEl, collectionEl.dataset.foundationCollectionId, includePreview);
        if (!currentColumn) {
            var columns = getColumns(collectionEl, includePreview);
    ...
    function handlePagination(state) {
        ...
        var f = function(e) {
            ...
            if (parentItemEl) {
                ...
                if (!loadPreview) {
                    ...
                    responseHandler = function(html) {
                        return extractColumn(html).then(function(newColumnEl) {
                            ...
                            var lazyLoader = setupLazyLoader(state, newColumnEl, src);
                ...
                if (showLoadingEl.getAttribute("is-loading")) {
                    return;
                } else {
                    showLoadingEl.setAttribute("is-loading", true);
                    responseHandler = function(html) {
                        return extractItems(html).then(function(result) {
                            ...
                            if (columnEl.dataset.foundationLayoutColumnviewLoadActiveItemEl === "true") {
                                ...
                                activeItemEl = itemArray.find(function(itemEl) {
                                    // eslint-disable-next-line max-len
                            ...
                            if (!lazyLoader) {
                                lazyLoader = setupLazyLoader(state, columnEl, src);
                            ...
                            if (collectionEl.dataset.foundationSelectionsSelectallMode === "true" &&
                                ...
                                selectItemsSilently($(itemArray)
                                    .filter(":not(.is-lazyLoaded):not(.is-pending)").toArray());
                ...
                $.ajax({
                    ...
                    .always(function() {
                        if (showLoadingEl) {
                            ...
                            showLoadingEl.removeAttribute("is-loading");
                        }
                    })
                    .then(responseHandler, function() {
                        var title = Granite.I18n.get("Error");
        ...
        if (src) {
            ...
            var lazyLoader = setupLazyLoader(state, currentColumnEl, src);
     ...
     * @returns {Object}
     */
    function setupLazyLoader(state, columnEl, src) {
        var LazyLoader = $(window).adaptTo("foundation-collection-lazyloader");
        var collection = state.collection;
     ...
     * @param {HTMLElement} collectionItem columnview child item with which user interacted
     * @param {Object} attributes optional extra information that needs to be sent
     */
    function trackEvent(trackAction, collection, config, collectionItem, attributes) {
        var trackElement = config.trackingElement;
            ...
            return {
                ...
                selectAll: function(suppressEvent) {
                    ...
                    var out = bulkSelect(state, function() {
                        ...
                        selectBulkItems(notSelected.toArray());

                        var selections = prevSelections.add(notSelected);

                        return {
                            // GRANITE-31274 selected all elements so prev selection does not matter
                            prevSelections: null,
                            selections: selections
                        };
                    });
...
(function(window, document, Granite, jquery, URITemplate) {
    ...
    function handleSelection(state) {
        ...
        var escape = function() {
            if (document.activeElement === collectionEl || collectionEl.contains(document.activeElement)) {
                collectionEl.dataset.foundationSelectionsSelectallMode = "false";
                collection.adaptTo("foundation-selections").clear();
            }
        };
    ...
    function handleSorting(state) {
        ...
        // make sure coral components are ready
        Coral.commons.ready(collectionEl, function() {
            if (collectionEl.dataset.foundationCollectionSortby &&
                ...
                count: function() {
                    if (collectionEl.dataset.foundationSelectionsSelectallMode === "true") {
                        ...
                        if (paginationAPI && paginationAPI.guessTotal !== -1) {
                            ...
                            // removing state not applicable to table layout
...
(function(window, $, Coral) {
    ...
    function createTooltip(placement) {
    ...
    function getLabel(label, errorId) {
        return label ? label + " " + errorId : errorId;
    }
    ...
    function getFieldError(el, field, message) {
        var error;
        var tooltip;
        ...
        if (!errorMap.has(el)) {
            ...
            if (form && form.classList.contains("coral-Form--vertical")) {
                placement = "left";
            }
            tooltip = createTooltip(placement);
            tooltip.target = error;
        ...
        var errorObj = errorMap.get(el);
        error = errorObj.error;
        tooltip = errorObj.tooltip;
        tooltip.content.innerHTML = Granite.UI.Foundation.XSS.sanitizeHtml(message);
        return errorObj;
    }
    ...
    $(window).adaptTo("foundation-registry").register("foundation.validation.validator", {
        ...
        show: function(el, message) {
            ...
            var errorObj = getFieldError(el, field, message);
            var error = errorObj.error;
            var tooltip = errorObj.tooltip;
...
(function(document, $) {
    ...
    $(document).on("click", ".granite-collection-deselect", function(e) {
        ...
        if (target) {
            ...
            var selectedItem = collection[0].selectedItem;
            // removing selectall mode and then clear selections
            delete collection[0].dataset.foundationSelectionsSelectallMode;
            api.clear();
...
(function(document, $, URITemplate) {
    ...
    $(document).on("change", "betty-breadcrumbs.granite-collection-navigator", function(e) {
        ...
        var target = this.dataset.graniteCollectionNavigatorTarget;
        var sanitizedTarget = Granite.UI.Foundation.XSS.sanitizeHtml(target);
        var collection = $(sanitizedTarget);
        ...
        if (collectionId) {
            ...
            var href = selectionEl.dataset.graniteCollectionNavigatorHref;
            if (new URL(href, document.baseURI).origin !== "null") {
                window.open(href);
            }
        }
    ...
    function updateNavigatorFromColumnView(control, collection) {
        ...
        var targetSelectItem = control.children("betty-breadcrumbs-item").filter(function() {
            return this.dataset.graniteCollectionNavigatorCollectionid === targetId ||
                (currentPathName === this.dataset.graniteCollectionNavigatorHref);
        });
